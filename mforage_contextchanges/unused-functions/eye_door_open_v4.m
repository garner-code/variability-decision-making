function [emr_cal, tgt_found, didx, startTime, door_on_flag] = eye_door_open_v4(REDm_info, trial_start, startTime, sub, sess, ...
    ...
    didx, fid, fform, xPos, yPos, r, fixCol, fixRect, maxFixDiam, emr_cal, t_until_target_onset)
...
if ~any(tgt_found)
 ...
 %   send_trigger(trg_id, sub, sess, trial_n, cond, vbl, ioObj, port_address, trglg_fid, trg_form);
    ...
    if didx == tgt_flag && (GetSecs - trial_start) > t_until_target_onset
        % record the door numbber into the results file
        door_on_flag = 0;
        d_p_idx = door_p(didx);
        tgt_found = 1;
        tgt_on = 1;
        fprintf(fid, fform, sub, sess, trial_n, cond, 999, 1, didx, d_p_idx, tgt_flag, tgt_on, tgt_found, .2, 9, 9);
                         % 'sub','sess','t', 'cond','onset','open_d','door','door_p','tgt_door','tgt_on','tgt_found','depress_dur','x','y'
        keep_door_open = 0;
        SMI_Redm_SendMessage(sprintf('%.3f_TgtFound', trial_start-GetSecs));
