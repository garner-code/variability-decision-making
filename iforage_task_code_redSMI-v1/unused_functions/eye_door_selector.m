function [didx, door_on_flag, threshold, new_startTime] = eye_door_selector(startTime, door, doorRects, tgt_flag, fid, sub, sess, trial_n, cond, trialStart, eye_used, el)
% this function tests whether the gaze has been on a single door for over a
% second, given samples collected every 10 ms
% also updates participant file every 10 ms
        ...
        if sum(idxs) > 0
            ...
            if didx == door % is it the same door?
                ...
                fprintf(fid, '%d\t%d\t%d\t%d\t%.3f\t%d\t%d\t%d\t%.3f\n', sub, sess, trial_n, cond, timer, 0, find(idxs), tgt_flag, dur);
                if dur > .5
                    threshold = 1;
                end
