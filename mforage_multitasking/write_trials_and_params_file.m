...
if stage == 1
    ...
    if sub < 10
        ...
        if stage == 3
            mt_trlfname   = sprintf('sub-0%d_ses-%s_task-mforage_mt-trls.tsv', sub, ses_str);
        end
    else
        trlfname   = sprintf('sub-%d_ses-%s_task-mforage_trls.tsv', sub, ses_str);
        if stage == 3
            mt_trlfname   = sprintf('sub-%d_ses-%s_task-mforage_mt-trls.tsv', sub, ses_str);
        end
    end
end

% define trial log file 
ncol2save = 5; % in all cases, we want to save 5 columns of trials to the trial log
...
fprintf(trlg_fid, '%d\t%d\t%d\t%d\t%d\t%d\t%d\n', [repmat(sub, 1, length(trials(:,1)))', repmat(stage, 1, length(trials(:,1)))', trials(:,1:ncol2save)]');
fclose(trlg_fid);
