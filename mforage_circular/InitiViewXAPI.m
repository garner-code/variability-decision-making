function REDm_info = InitiViewXAPI(envProfile)
% InitiViewXAPI.m
%
...
% Added the envProfile input as implemented by DavidPainter D.Lloyd 9/2018....
...
% a profile name 
REDGeometryData.setupName = int8(envProfile.setupName);  

% Visible Screen width [mm]  MEASURED for station1
REDGeometryData.stimX = int32(envProfile.screenXmm); 

% Visible Screen height [mm] MEASURED for station1
REDGeometryData.stimY = int32(envProfile.screenYmm); 

% Vertical distance RED-m to stimulus screen [mm]  MEASURED for station1
REDGeometryData.redStimDistHeight = int32(envProfile.red2screenVmm); 

% Horizontal distance RED-m to stimulus screen [mm]  MEASURED for station1
REDGeometryData.redStimDistDepth = int32(envProfile.red2screenHmm); 

% off horizontal RED-m inclination angle [degree]  MEASURED for station1
REDGeometryData.redInclAngle = int32(envProfile.red2screenAdeg); 
...
CalibrationData.method = int32(9); % number of data points in calibration test pattern 2,5,8,9,13 (default: 5)
...
CalibrationData.displayDevice = int32(0); % 0 - primary, 1 - secondary
...
CalibrationPointData.number = int32(0);
CalibrationPointData.positionX = int32(0);
CalibrationPointData.positionY = int32(0);
pCalibrationPointData = libpointer('CalibrationPointStruct', CalibrationPointData);
...
REDm_info.pCalibrationPointData = pCalibrationPointData;
