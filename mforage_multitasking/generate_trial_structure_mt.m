...
% counterbalancing of baits vs tgt locations vs tgt ids == 128 trials, therefore
...
tntrials = ntrials/n_repeat_base*ncontexts;% this assumes only 2 contexts, the
% hardcoded 2 would need to change for experiments involving more than 2.
trial_struct = zeros(tntrials, 10);  
...
% and mt location
% target
ca = repmat(repelem(ca_idxs, length(ca_idxs)*length(ca_idxs)),1,2);
cb = repmat(repelem(cb_idxs, length(ca_idxs)*length(ca_idxs)),1,2);
...
for i = 1:length(ca_idxs)
    ca_trigger(ca == ca_idxs(i)) = ...
        repmat([0, ca_idxs(ca_idxs ~= ca_idxs(i))], ...
                            1, length(ca_idxs)*n_mt_tgt_types); % * 2 as 2 target identities
    cb_trigger(cb == cb_idxs(i)) = ...
        repmat([0, cb_idxs(cb_idxs ~= cb_idxs(i))], ...
                            1, length(cb_idxs)*n_mt_tgt_types);
end

% location for multitask | ca
ca_mt_loc = repmat(repelem(cb_idxs, length(ca_idxs)), 1, length(ca_idxs) * n_mt_tgt_types);
cb_mt_loc = repmat(repelem(ca_idxs, length(cb_idxs)), 1, length(cb_idxs) * n_mt_tgt_types);
mt_tgt_type = repelem([1,2],n_base/n_mt_tgt_types);
...
trial_struct(:,9) = 1;
trial_struct(:,10) = repmat(mt_tgt_type, 1, ncontexts);
